# -*- coding: utf-8 -*-
"""
Created on Thu May 26 09:58:46 2016
...
def mccv(data, labels, classifier, n_iter, test_size, metric):
    ...
    for train_index, test_index in splits :
        ...
        accs.append(metrics.roc_auc_score(labels_test, classifier.predict(K_test)))
...
for i in range(-10, 10) :
    ...
    for j in range(1, 20):
        ...
        K = np.genfromtxt(kernel_dir + 'K_rw_chisq_ ' + str(i) + '_' + str(j) + '.csv', delimiter=',')
...
grid_search_results = np.zeros((20, 19))
...
for train_index, test_index in kf:
    ...
    parameter_results = np.zeros((20, 19))
    ...
    for i in range(-10, 10) :
        ...
        for j in range(1, 20) :
            ...
            parameter_results[i + 10, j -1 ] = np.mean(mccv(K_fold_train, labels_fold_train, clf, n_reps, 0.1, 'accuracy_score'))
    ...
    key = (2 ** (best_parameters[0] - 10)) * (3 ** (best_parameters[1] + 1))
    print 'best_parameters = ' + str(best_parameters)
    print 'best value of gamma is ' + str(2 ** (best_parameters[0] - 10))    
    print 'best value of sigma is ' + str(2 ** (best_parameters[1] +1 ))     
    K = kernel_dict[key]
...
for i in range(-10, 10) :
    ...
    for j in range(1, 20) :
        ...
        best_results[i + 10, j-1] = metrics.accuracy_score(preds, labels)
        
print best_results
np.savetxt(kernel_dir + 'best_results.csv', best_results, delimiter=',')
